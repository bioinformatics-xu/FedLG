...
from model import *
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData, kaggle, heart ,coronary01, coronary1, coronary2, coronary10
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if dataset == 'mnist':
        ...
        np.save(datadir + "/y_train.npy", y_train)
        np.save(datadir + "/y_test.npy", y_test)
    elif datadir == "./data/coronary01":
        print("")
        cell_features = pd.read_csv(datadir + '/coronary01_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/coronary01_lable.csv')
    ...
    if partition == "homo":
        ...
        min_require_size = 10
        K = 10
        if dataset in ('celeba', 'covtype', 'a9a', 'rcv1', 'SUSY', 'heart', 'kaggle'):
            K = 2
            # min_require_size = 100
        if dataset in ('coronar1'):
            K = 2
        if dataset == 'cifar100':
            K = 100
        elif dataset == 'tinyimagenet':
